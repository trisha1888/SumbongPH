...
const getPriorityStyle = (priority: string) => {
  switch (String(priority).trim().toLowerCase()) {
    ...
    case 'low': default: return { bg: '#F3F4F6', text: '#374151' };
  }
};
...
const getStatusStyle = (status: string) => {
  switch (normalizeStatus(status)) {
    ...
    case 'Pending': default: return { bg: '#FEF3C7', text: '#B45309' };
  }
};
...
const ComplaintsDashboard = () => {
  ...
  const [pickerConfig, setPickerConfig] = useState<{ title: string; options: string[]; current: string; onSelect: (val: string) => void } | null>(null);
  ...
  const filteredData = useMemo(() => {
    return complaintsData.filter((item) => {
      ...
      const matchesSearch = !query || item.reportCode.toLowerCase().includes(query) || 
  ...
  const openPicker = (title: string, options: string[], current: string, setter: (val: string) => void) => {
...
<TouchableOpacity onPress={() => router.push('/(admin.dashboard)/announcements.dashboard')}><Text style={styles.navItem}>Announcements</Text></TouchableOpacity>
        ...
        <View style={styles.filtersCard}>
          ...
          <View style={styles.filterChipsRow}>
            ...
            {/* Reset Button */}
            {(category !== 'All Categories' || status !== 'All Statuses' || priority !== 'All Priorities') && (
              <TouchableOpacity onPress={() => { setCategory('All Categories'); setStatus('All Statuses'); setPriority('All Priorities'); }}>
        ...
        <View style={styles.tableCard}>
          ...
          {loading ? (
            ...
            <View style={styles.noResults}><Text style={styles.noResultsText}>No complaints found.</Text></View>
          ) : (
              ...
              <View key={item.id} style={styles.tableRow}>
                ...
                <View style={styles.badgeContainer}>
                  <View style={[styles.badge, { backgroundColor: getPriorityStyle(item.priority).bg }]}>
                    <Text style={[styles.badgeText, { color: getPriorityStyle(item.priority).text }]}>{item.priority}</Text>
                  </View>
                </View>
                <View style={styles.badgeContainer}>
                ...
                <View style={styles.actionsCell}>
                  <TouchableOpacity style={styles.actionBtn} onPress={() => { setSelectedComplaint(item); setStatusModalVisible(true); }}>
      ...
      {/* REUSABLE SELECTION MODAL (For Category, Status, Priority) */}
        ...
        <Pressable style={styles.modalOverlay} onPress={() => setPickerModalVisible(false)}>
          <View style={styles.modalCard}>
            ...
            {pickerConfig?.options.map((option) => (
              <TouchableOpacity 
                key={option} 
                style={[styles.statusChoiceBtn, pickerConfig.current === option && styles.statusChoiceActive]}
                onPress={() => { pickerConfig.onSelect(option); setPickerModalVisible(false); }}
              >
                <Text style={[styles.statusChoiceText, pickerConfig.current === option && { color: '#FF6B00' }]}>{option}</Text>
                {pickerConfig.current === option && <Ionicons name="checkmark" size={18} color="#FF6B00" />}
              </TouchableOpacity>
            ))}
          </View>
        </Pressable>
        ...
        <Pressable style={styles.modalOverlay} onPress={() => setStatusModalVisible(false)}>
          <View style={styles.modalCard}>
            ...
            {STATUS_CHOICES.map((s) => (
              ...
              >
                {updatingId === selectedComplaint?.id ? <ActivityIndicator size="small" color="#FF6B00" /> : <Text style={styles.statusChoiceText}>{s}</Text>}
              </TouchableOpacity>
            ))}
          </View>
        </Pressable>
...
// ... Styles (included only modified/new ones for brevity)
const styles = StyleSheet.create({
  safeArea: { flex: 1, backgroundColor: '#FFF' },
  container: { flex: 1 },
  mobilePadding: { padding: 20 },
  desktopPadding: { paddingHorizontal: '10%', paddingVertical: 40 },
  navBar: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 40 },
  ...
  filtersCard: { borderWidth: 1, borderColor: '#E5E7EB', borderRadius: 12, padding: 16, marginBottom: 16, backgroundColor: '#FFF' },
  searchRow: { flexDirection: 'row', alignItems: 'center', borderWidth: 1, borderColor: '#E5E7EB', borderRadius: 10, paddingHorizontal: 12, height: 44 },
  searchInput: { flex: 1, marginLeft: 8, color: '#111827' },
  filterChipsRow: { flexDirection: 'row', gap: 10, marginTop: 12, flexWrap: 'wrap', alignItems: 'center' },
  filterChip: { flexDirection: 'row', alignItems: 'center', gap: 6, backgroundColor: '#F9FAFB', borderWidth: 1, borderColor: '#E5E7EB', paddingHorizontal: 12, paddingVertical: 8, borderRadius: 999 },
  ...
  tableCard: { borderWidth: 1, borderColor: '#E5E7EB', borderRadius: 12, overflow: 'hidden', backgroundColor: '#FFF' },
  ...
  complaintCell: { flexDirection: 'row', alignItems: 'center', flex: 2.4, marginRight: 8 },
  ...
  modalCard: { width: '100%', maxWidth: 350, backgroundColor: '#FFF', borderRadius: 16, padding: 20 },
  modalTitle: { fontSize: 16, fontWeight: '800', marginBottom: 15 },
  statusChoiceBtn: { flexDirection: 'row', justifyContent: 'space-between', padding: 15, borderBottomWidth: 1, borderBottomColor: '#F3F4F6' },
